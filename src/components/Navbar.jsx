...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 50) {
        setScrolling(true); // Au-delà de 50px, on fait défiler
      } else {
        setScrolling(false); // Revenir à la normale
      }
    };
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all ${scrolling ? 'bg-black/90' : 'bg-transparent'}`}>
      <div className="container mx-auto px-10">
        <div className="flex items-center justify-between">

          {/* Menu gauche visible en desktop uniquement */}
          <div className={`hidden lg:flex space-x-8 ${scrolling ? 'hidden' : ''}`}>
          ...
          <div className={`flex justify-center items-center w-full transition-all ${scrolling ? 'absolute right-16 top-0 transform translate-x-1/2' : ''}`}>
            <img
              src={gLogo}
              alt="G Logo"
              className={`h-12 w-12 md:h-24 md:w-24 opacity-90 hover:opacity-100 transition-all ${scrolling ? 'cursor-pointer' : ''}`}
              onClick={toggleMenu}  // Ouvre le menu au clic
            />
          </div>
          ...
          <div className={`hidden lg:flex space-x-10 items-center ${scrolling ? 'hidden' : ''}`}>
